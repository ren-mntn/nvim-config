...
return {
  ...
  config = function()
    ...
    -- カスタムハイライト設定
    vim.api.nvim_set_hl(0, "Comment", { fg = colors.comment }) -- コメントをグレーに
    ...
    -- TypeScript固有のSyntaxハイライト
    vim.api.nvim_set_hl(0, "typescriptBraces", { fg = colors.red }) -- TypeScript波括弧を赤色
    
    -- Treesitter波括弧ハイライト（優先度が高いため追加）
    vim.api.nvim_set_hl(0, "@punctuation.bracket", { fg = colors.highYellow }) -- TSX波括弧を赤色
    
    -- LSP semantic tokensでの変数色
    vim.api.nvim_set_hl(0, "@lsp.type.variable.typescriptreact", { fg = colors.blue2 })
    vim.api.nvim_set_hl(0, "@lsp.typemod.variable.local.typescriptreact", { fg = colors.blue2 })
  end,
}
