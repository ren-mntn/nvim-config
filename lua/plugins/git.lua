return {
  -- Git操作の拡張（Telescopeとsnacks併用）
  {
    ...
    keys = {
      ...
      { "<leader>gh", "<cmd>Telescope git_commits<cr>", desc = "Git History (all commits)" },
      { "<leader>gH", "<cmd>Telescope git_bcommits<cr>", desc = "Git History (current file)" },
      ...
      end, desc = "Switch to main branch" },
  ...
  -- fugitive.vimでGitコマンドを強化
  {
    ...
    keys = {
      ...
      { "<C-Enter>", function()
        ...
        vim.cmd("Git commit")
      end, desc = "Commit Staged Changes (VSCode)" },
      
      { "<C-S-Enter>", function()
        -- 全変更をステージしてコミット（VSCode風）
        vim.cmd("Git add .")
        vim.cmd("Git commit")
      end, desc = "Stage All & Commit (VSCode)" },
